...
type ModalType = 'purchase' | 'adjust' | 'check' | 'consume' | 'add' | 'edit' | null
type FilterStatus = 'all' | 'critical' | 'warning' | 'ok'

const MOVEMENT_META: Record<string, { label: string; color: string; icon: string }> = {
  purchase:   { label: 'Compra',   color: 'bg-blue-100 text-blue-600',   icon: '➕' },
...
export default function StockPage() {
  ...
  const [mvFilter, setMvFilter] = useState<'all' | 'purchase' | 'adjustment' | 'consumption'>('all')
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => openModal('add')} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">+ Ingrediente</button>
          <button onClick={() => openModal('purchase')} className="text-xs px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors font-medium">➕ Compra</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          { icon: '➕', label: 'Compra', sub: 'Suma al stock', border: 'border-blue-200 hover:bg-blue-50', action: () => openModal('purchase') },
          { icon: '🍽', label: 'Consumo', sub: 'Uso en producción', border: 'border-green-200 hover:bg-green-50', action: () => openModal('consume') },
          { icon: '⚡', label: 'Ajuste', sub: 'Merma o corrección', border: 'border-orange-200 hover:bg-orange-50', action: () => openModal('adjust') },
          { icon: '🔍', label: 'Control', sub: 'Conteo parcial', border: 'border-purple-200 hover:bg-purple-50', action: () => openModal('check') },
      ...
      {activeTab === 'stock' && (
        <>
          <div className="flex flex-wrap gap-2 mb-4">
            <input type="text" placeholder="Buscar..." className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm flex-1 min-w-40 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white"
      ...
      {/* Modales */}
      {modal === 'purchase' && <PurchaseModal ingredients={ingredients} onClose={closeModal} onSuccess={() => { load(); showToast('Compra registrada') }} />}
      {modal === 'consume'  && <ConsumptionModal ingredients={ingredients} onClose={closeModal} onSuccess={() => { load(); showToast('Consumo registrado') }} defaultIngredientId={selectedId} />}
