...
function IngredientDetail({ ingredient: i, onClose, onAdjust, onCheck, onConsume, onTransfer, onEdit }: {
  ingredient: IngredientWithStatus
  onClose:    () => void
  onAdjust:   (id: string) => void
  onCheck:    (id: string) => void
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-blue-50 border border-blue-200 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-blue-400 mt-0.5">{i.unit}</p>
          </div>

          <div className="flex flex-col items-center gap-1">
            <span className="text-xs text-gray-400">pase</span>
            <span className="text-gray-300 text-lg">→</span>
          </div>

          <div className="flex-1 bg-orange-50 border border-orange-200 rounded-xl p-3 text-center">
            <p className="text-xs text-orange-600 font-medium mb-1">📦 Cajón</p>
            <p className="text-xl font-semibold text-orange-900">{fmt(stockCajon)}</p>
            <p className="text-xs text-orange-400 mt-0.5">{i.unit}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => { onTransfer(i.id); onClose() }}
            className="py-2 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 rounded-lg transition-colors col-span-2">
            🔄 Pasar Freezer → Cajón
          </button>
...
export function IngredientCard({ ingredient: i, onAdjust, onCheck, onConsume, onTransfer, onEdit, onDelete }: Props) {
  const s            = STATUS[i.status]
  const stockCajon   = (i as any).stock_cajon   ?? 0
  const stockFreezer = (i as any).stock_freezer ?? 0
  const pct = i.stock_min > 0 ? Math.min(100, (i.stock_current / (i.stock_min * 2)) * 100) : 100
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            <div className={`rounded-lg px-2 py-1 flex items-center justify-between ${stockCajon <= 0 ? 'bg-red-50' : 'bg-orange-50'}`}>
              <span className="text-xs text-orange-500">📦</span>
              <span className={`text-xs font-semibold ${stockCajon <= 0 ? 'text-red-600' : 'text-orange-700'}`}>
                {fmt(stockCajon)}
              </span>
            </div>
          ...
          <div className="flex gap-1.5 mb-1.5" onClick={e => e.stopPropagation()}>
            <button onClick={() => onTransfer(i.id)}
              className="flex-1 py-1.5 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 rounded-lg transition-colors">
              🔄 Pase
            </button>
          ...
          <div className="flex gap-1.5" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => onDelete(i.id)}
              className="py-1.5 px-2.5 text-xs font-medium text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              ✕
            </button>
          </div>
        </div>
