...
// ─── COMPRA ──────────────────────────────────────────────────
export function PurchaseModal({ ingredients, onClose, onSuccess }: { ingredients: IngredientWithStatus[]; onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleSubmit() {
    ...
    try {
      await createPurchase({ ingredient_id: form.ingredient_id, quantity: parseFloat(form.quantity), unit_price: parseFloat(form.unit_price) || 0, total_cost: total || 0, supplier: form.supplier || undefined })
      onSuccess(); onClose()
  ...
  return (
    <Modal title="➕ Registrar Compra" onClose={onClose}>
      <div className="space-y-4">
        ...
        {total !== null && total > 0 && <div className="bg-blue-50 rounded-lg px-4 py-2 text-sm text-blue-700">Total: <strong>{fmtCurrency(total)}</strong></div>}
        <div>
...
export function AdjustModal({ ingredients, onClose, onSuccess, defaultIngredientId }: { ingredients: IngredientWithStatus[]; onClose: () => void; onSuccess: () => void; defaultIngredientId?: string }) {
  const [form, setForm] = useState({ ingredient_id: defaultIngredientId || '', type: 'subtract' as 'add' | 'subtract', quantity: '', reason: 'ajuste' as AdjustmentReason, notes: '' })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const selected = ingredients.find(i => i.id === form.ingredient_id)
  const preview = selected && form.quantity ? Math.max(0, selected.stock_current + (form.type === 'add' ? 1 : -1) * parseFloat(form.quantity)) : null
  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }))
  ...
  async function handleSubmit() {
    ...
    setLoading(true); setError('')
    try {
      await createAdjustment({ ingredient_id: form.ingredient_id, type: form.type, quantity: parseFloat(form.quantity), reason: form.reason, notes: form.notes || undefined })
      onSuccess(); onClose()
    } catch (e: any) { setError(e.message) } finally { setLoading(false) }
  }
    ...
    <Modal title="⚡ Ajuste Rápido" onClose={onClose}>
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(['subtract', 'add'] as const).map(t => (
              <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))}
                className={`py-2 rounded-lg text-sm font-medium border transition-all ${form.type === t ? (t === 'subtract' ? 'bg-red-50 border-red-300 text-red-700' : 'bg-green-50 border-green-300 text-green-700') : 'border-gray-200 text-gray-500'}`}>
        ...
        {preview !== null && selected && (
          <div className={`rounded-lg px-4 py-2 text-sm font-medium ${form.type === 'subtract' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
        ...
        <div>
          ...
          <input type="text" className="input-base" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Opcional" />
        </div>
        {error && <p className="text-red-500 text-xs">{error}</p>}
        <button onClick={handleSubmit} disabled={loading} className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
...
// ─── CONTROL ─────────────────────────────────────────────────
export function CheckModal({ ingredients, onClose, onSuccess, defaultIngredientId }: { ingredients: IngredientWithStatus[]; onClose: () => void; onSuccess: () => void; defaultIngredientId?: string }) {
    ...
    <Modal title="🔍 Control de Stock" onClose={onClose}>
      <div className="space-y-4">
        ...
        {Math.abs(saved.difference) > 0.001 && (
          <button onClick={handleApply} disabled={applying} className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
            {applying ? 'Aplicando...' : '✓ Ajustar stock automáticamente'}
          </button>
        )}
        <button onClick={onClose} className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg py-2.5 text-sm transition-colors">Cerrar sin ajustar</button>
        {error && <p className="text-red-500 text-xs">{error}</p>}
      </div>
    </Modal>
  )
  ...
  return (
    <Modal title="🔍 Control de Stock" onClose={onClose}>
      <div className="space-y-4">
        ...
        {selected && <div className="bg-gray-50 rounded-lg px-4 py-2.5 text-sm"><span className="text-gray-500">Sistema: </span><strong>{fmt(selected.stock_current)} {selected.unit}</strong></div>}
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Cantidad real contada {selected ? `(${selected.unit})` : ''} *</label>
          <input type="number" min="0" step="0.001" className="input-base" value={form.real_quantity} onChange={e => set('real_quantity', e.target.value)} placeholder="0.00" />
        </div>
        {diff !== null && (
        ...
        {error && <p className="text-red-500 text-xs">{error}</p>}
        <button onClick={handleSubmit} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            ...
            <input type="text" className="input-base" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Ej: Harina 000" autoFocus />
          </div>
          <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {([{k:'stock_current',l:'Stock inicial'},{k:'stock_min',l:'Stock mínimo'},{k:'cost_unit',l:'Costo/unidad ($)'}]).map(({k,l}) => (
            <div key={k}>
              ...
              <input type="number" min="0" step="0.01" className="input-base" value={(form as any)[k]} onChange={e => set(k, e.target.value)} placeholder="0" />
            </div>
          ))}
        </div>
        {error && <p className="text-red-500 text-xs">{error}</p>}
        <button onClick={handleSubmit} disabled={loading} className="w-full bg-gray-900 hover:bg-gray-800 disabled:opacity-50 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
